...
int Binp(int a[], int b, int e, int key)	
{
	...
	if (b == e) return b;	// лева¤ и правая граница совпали, 
							// и это и есть индекс искомого элемента
	int m = (b + e) / 2;
	if (a[m] == key) return m;	// число key находится на месте m
	else if (a[m] > key) Binp(a, b, m - 1, key);	// число key находится в левой половине
													// следовательно сдвигаем правую границу e
	else/*(a[m] < key)*/ Binp(a, m + 1, e, key);	// число key находится в правой половине
